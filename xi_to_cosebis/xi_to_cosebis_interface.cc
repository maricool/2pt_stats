	...
	int get_option(cosmosis::DataBlock * options, const string &name, string &parameter)
	{
  	...
  	void * setup(cosmosis::DataBlock * options, cosmosis::DataBlock * block)
  	{
		...
		const DATABLOCK_STATUS failure = (DATABLOCK_STATUS)1;
  		...
  		status=options->get_val<string>(sectionName, string("output_section_name"), string("cosebis_2PCFs"), 
				...
				if(ShouldIwriteTofile)
				{
					...
					if (config->nZBins<=0)
					{
						...
						if(config->range)
						{
							...
							name_En=string("EnFrom2PCFs_LOS")+toString(isim);
							name_Bn=string("BnFrom2PCFs_LOS")+toString(isim);
						}
						else
						{
							name_En=string("EnFrom2PCFs");
							name_Bn=string("BnFrom2PCFs");
						}
		...
		else // this needs to be updated because now xi_plus and minus are in different sections
		{
			...
			for (int i_bin=1; i_bin<=num_z_bin_A; i_bin++) 
			{
				for (int j_bin=1; j_bin<=num_z_bin_B; j_bin++) 
				{
					...
					string name_in="bin_"+toString(i_bin)+"_"+toString(j_bin);
		    		status = block->get_val(config->input_section_name_plus, name_in, xiP_vec);
					if (status) 
					{
						clog<<"Could not load bin "<<i_bin<<"_"<< j_bin<<" in XiP to COSEBIs"<<endl;
						return status;
					}
		    		status = block->get_val(config->input_section_name_minus, name_in, xiM_vec);
					if (status) 
					{
						clog<<"Could not load bin "<<i_bin<<"_"<< j_bin<<" in XiM to COSEBIs"<<endl;
						return status;
					}
